...
filter_transcriptome_gtf  <-  function( gtf , filter = TRUE, filter_column = "gene_name", filter_list=c('Gm43552','Nuak2','Nid1')) {
  
  if(length(filter_list) > 0)
{
  if (filter==TRUE & filter_column == "gene_name")
  {
    gtf <- gtf |>
      filter(!gene_name %in% filter_list)
  } 
  
  if (filter==TRUE & filter_column == "gene_id")
  {
    gtf <- gtf |>
      filter(!gene_id %in% filter_list)
  } 
  
  if (filter==TRUE & filter_column == "transcript_id")
  {
    gtf <- gtf |>
      filter(!transcript_id %in% filter_list)
  } 
  
  return(gtf)	
}
else {
  cat("Vector is empty !!!! There is no gene_name, gene_id or transcript_id")
}



} # filter_transcriptome_gtf

#high_utr <- c('Gm43552','Nuak2','Nid1','Cox8a','Cep162','Tmem38b','Tex52','Smim10l1','Gm38394','Chtf8','Glod4','Lrrc58','Elovl1','Aars','D10Wsu102e','Eef1a1','Lars2')
#filter_transcriptome_gtf(myData.transcriptome.gtf, filter = TRUE, filter_column = "gene_name", filter_list = high_utr)|>
#  dim()


################################################################################
################################################################################
## Select specific genes (EMT, Ribosome etc.)
select_transcriptome_gtf  <-  function( gtf , filter = TRUE, filter_column = "gene_name", filter_list=c('Gm43552','Nuak2','Nid1')) {
...
#high_utr <- c('Gm43552','Nuak2','Nid1','Cox8a','Cep162','Tmem38b','Tex52','Smim10l1','Gm38394','Chtf8','Glod4','Lrrc58','Elovl1','Aars','D10Wsu102e','Eef1a1','Lars2')
#select_transcriptome_gtf(myData.transcriptome.gtf, filter = TRUE, filter_column = "gene_name", filter_list = high_utr)|>
#  dim()
...
# build_gene_set_with_riboseq_selected_genelist
# input:
#   pattern_rnaseq_gene: (e.g. n-R5s)
#   pattern_riboseq_gene: (e.g. n-R5s)
#   genelist=c("Tp53", "Rps6") # List of genes for motif analysis
# output:
#   gene_set: list
#   gene_set$sets: list of list
#     gene_set$sets[['unt48.tgfb48']]$up
...
build_gene_set_with_riboseq_selected_genelist <- function(project_rnaseq="211613", genelist=selected_genes, project_riboseq="161021", rundata_appendix=".rdna_rn18s", individual="NMuMG", level="htseq_gene", f_include_tgfb_vs_cx5461=TRUE, pattern_rnaseq_gene=NULL, pattern_riboseq_gene=NULL) {
  ...
  {
    rnaseq.deTest <- rnaseq.deTest |>
      filter(gene_name %in% genelist)
    
    riboseq.deTest <- riboseq.deTest |>
      filter(gene_name %in% genelist)
  }
  ...
  # gene_set_rnaseq
  gene_set <- NULL
  gene_set <- build_gene_set(de = rnaseq.deTest, project = project_rnaseq, conditions = c("unt48", "tgfb48"), gene_set = gene_set, gname = "unt48.tgfb48.DEtranscription", verbose = T)
  if (f_include_tgfb_vs_cx5461) {
    gene_set <- build_gene_set(de = rnaseq.deTest, project = project_rnaseq, conditions = c("tgfb48", "tgfbCX5461"), gene_set = gene_set, gname = "tgfb48.tgfbCX5461.DEtranscription", verbose = T)
  }
  gene_set <- build_gene_set(de = rnaseq.deTest, project = project_rnaseq, conditions = c("unt48", "tgfb48", "tgfbCX5461"), gene_set = gene_set, gname = "reversible.transcription.CX", verbose = T)
  gene_set_rnaseq <- NULL
  gene_set_rnaseq <- gene_set
...
#RibosomalPrpteins <- c("Rpsa", "Rps2", "Rps3", "Rps3a", "Rps4x", "Rps5", "Rps6", "Rps7", "Rps8", "Rps9", "Rps10", "Rps11", "Rps12", "Rps13", "Rps14", "Rps15", "Rps15a", "Rps16", "Rps17", "Rps18", "Rps19", "Rps20", "Rps21", "Rps23", "Rps24", "Rps25", "Rps26", "Rps27", "Rps27a", "Rps28", "Rps29", "Fau", "Rpl3", "Rpl4", "Rpl5", "Rpl6", "Rpl7", "Rpl7a", "Rpl8", "Rpl9", "Rpl10", "Rpl10a", "Rpl11", "Rpl12", "Rpl13", "Rpl13a", "Rpl14", "Rpl15", "Rpl17", "Rpl18", "Rpl18a", "Rpl19", "Rpl21", "Rpl22", "Rpl23", "Rpl23a", "Rpl24", "Rpl26", "Rpl27", "Rpl27a", "Rpl28", "Rpl29", "Rpl30", "Rpl31", "Rpl32", "Rpl34", "Rpl35", "Rpl35a", "Rpl36", "Rpl44", "Rpl37", "Rpl37a", "Rpl38", "Rpl39", "Rpl40", "Rpl41", "Rplp0", "Rplp1", "Rplp2")
#gene_set_ribosomalProteins <- build_gene_set_with_riboseq_selected_genelist()
